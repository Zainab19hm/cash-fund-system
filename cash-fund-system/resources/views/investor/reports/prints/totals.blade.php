...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .totals-grid { display: flex; gap: 40px; justify-content: center; margin: 30px 0; }
        .totals-grid .total-card { flex: 1; max-width: 400px; text-align: center; padding: 25px; border: 1px solid #e5e7eb; border-radius: 12px; }
        .totals-grid .total-card .big-label { margin-bottom: 8px; }
        .totals-grid .total-card .big-number { margin: 0; font-size: 32px; }
        .payment-color { color: #dc2626; }
        .receipt-color { color: #16a34a; }
    </style>
</head>
<body>
    <div class="no-print">
    ...
    <div class="page">
        ...
        <div class="totals-grid">
            <div class="total-card">
                <div class="big-label">إجمالي الصرف</div>
                <div class="big-number payment-color">{{ number_format($totals['payment'] ?? 0, 2) }} د.ك</div>
            </div>
            <div class="total-card">
                <div class="big-label">إجمالي القبض</div>
                <div class="big-number receipt-color">{{ number_format($totals['receipt'] ?? 0, 2) }} د.ك</div>
            </div>
        </div>
