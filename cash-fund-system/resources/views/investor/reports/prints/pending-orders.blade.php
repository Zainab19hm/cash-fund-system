...
<head>
    <meta charset="utf-8">
    <title>الأوامر المعلقة — نظام إدارة الصندوق النقدي</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .type-payment { color: #dc2626; font-weight: 600; }
        .type-receipt { color: #16a34a; font-weight: 600; }
    </style>
</head>
<body>
    <div class="no-print">
            ...
            <tbody>
                @forelse($orders as $index => $order)
                    <tr>
                        ...
                        <td class="{{ $order->type === 'payment' ? 'type-payment' : 'type-receipt' }}">
                            {{ $order->type === 'payment' ? 'صرف' : 'قبض' }}
                        </td>
                        <td>{{ number_format($order->amount, 2) }} د.ك</td>
                        <td>{{ $order->creator->name ?? '—' }}</td>
                        <td>{{ $order->created_at->format('Y-m-d H:i') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="no-data">لا توجد أوامر معلقة</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
