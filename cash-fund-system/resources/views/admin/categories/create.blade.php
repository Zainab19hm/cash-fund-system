<x-app-layout title="تصنيف جديد">
    <div class="mx-auto max-w-2xl space-y-6">

        {{-- Page Header --}}
        <div>
            <h1 class="font-heading text-2xl font-bold text-primary">تصنيف جديد</h1>
            <p class="mt-1 text-sm text-muted">إضافة تصنيف جديد لبنود الطلبات</p>
        </div>
        ...
        {{-- Form --}}
        <form method="POST" action="{{ route('admin.categories.store') }}" class="rounded-xl border border-bdr bg-surface p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="type" id="type" required
                    ...
                    <option value="both" {{ old('type') === 'both' ? 'selected' : '' }}>صرف وقبض</option>
                </select>
                @error('type')
                    <p class="mt-1.5 text-xs text-red-400">{{ $message }}</p>
                @enderror
