<x-app-layout title="سجل التدقيق">
    <div class="space-y-6">
        <x-admin-nav />

        <div class="flex items-center justify-between">
            <div>
                <h1 class="font-heading text-2xl font-bold text-primary">سجل التدقيق</h1>
                <p class="mt-1 text-sm text-muted">الطلبات التي تم اعتمادها أو رفضها</p>
            </div>
            <a href="{{ route('admin.reports.audit-trail.print', request()->query()) }}"
               target="_blank"
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-bdr bg-bg/50">
                            ...
                            <th class="px-4 py-3 text-right font-semibold text-muted">الطالب</th>
                            <th class="px-4 py-3 text-right font-semibold text-muted">المعتمد/المرفوض</th>
                            <th class="px-4 py-3 text-right font-semibold text-muted">المنفذ</th>
                        ...
                        @forelse ($orders as $order)
                            <tr class="transition-colors hover:bg-bg/50">
                                ...
                                <td class="px-4 py-3 text-muted">
                                    @if ($order->approver)
                                        {{ $order->approver->name }}
                                    @elseif ($order->rejector)
                                        {{ $order->rejector->name }}
                                    @else
                                        —
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-muted">{{ $order->executor->name ?? '—' }}</td>
                                <td class="px-4 py-3 text-muted">
