<x-app-layout title="إجمالي الصرف والقبض">
    <div class="space-y-6">
        <x-admin-nav />

        <div class="flex items-center justify-between">
            <div>
                <h1 class="font-heading text-2xl font-bold text-primary">إجمالي الصرف والقبض</h1>
            </div>
            <a href="{{ route('admin.reports.totals.print', request()->query()) }}"
               target="_blank"
        ...
        <form method="GET" action="{{ route('admin.reports.totals') }}" class="rounded-xl border border-bdr bg-surface p-4">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl border border-bdr bg-surface p-6 text-center">
                <div class="text-sm font-semibold text-muted mb-2">إجمالي الصرف</div>
                <div class="text-3xl font-bold text-red-400">{{ number_format($totals['payment'], 2) }}</div>
            </div>
            <div class="rounded-xl border border-bdr bg-surface p-6 text-center">
                <div class="text-sm font-semibold text-muted mb-2">إجمالي القبض</div>
                <div class="text-3xl font-bold text-green-400">{{ number_format($totals['receipt'], 2) }}</div>
            </div>
        </div>
    </div>
</x-app-layout>
