...
<head>
    <meta charset="utf-8">
    <title>حالة أرشفة الوثائق — نظام إدارة الصندوق النقدي</title>
            ...
            <tbody>
                ...
                @forelse($orders as $order)
                    <tr>
                        ...
                        <td>{{ $order->documents_count }}</td>
                        <td>{{ $order->creator->name }}</td>
                        <td>{{ $order->order_date->format('Y-m-d') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="7" style="text-align: center;">لا توجد وثائق</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
