<x-app-layout title="كشف الحركة لفترة">
    <div class="space-y-6">
        <x-admin-nav />

        <div class="flex items-center justify-between">
            <div>
                <h1 class="font-heading text-2xl font-bold text-primary">كشف الحركة لفترة</h1>
            </div>
            <a href="{{ route('admin.reports.movement-statement.print', request()->query()) }}"
               target="_blank"
        ...
        <form method="GET" action="{{ route('admin.reports.movement-statement') }}" class="rounded-xl border border-bdr bg-surface p-4">
        ...
        <div class="rounded-xl border border-bdr bg-surface p-12 text-center">
            <p class="text-muted">اختر الفترة الزمنية ثم اضغط عرض</p>
        </div>
        @endif
    </div>
</x-app-layout>
