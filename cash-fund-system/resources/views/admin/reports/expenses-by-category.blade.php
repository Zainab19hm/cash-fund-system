<x-app-layout title="الصرف حسب البند">
    <div class="space-y-6">
        <x-admin-nav />

        <div class="flex items-center justify-between">
            <div>
                <h1 class="font-heading text-2xl font-bold text-primary">الصرف حسب البند</h1>
                <p class="mt-1 text-sm text-muted">فقط الطلبات EXECUTED</p>
            </div>
            <a href="{{ route('admin.reports.expenses-by-category.print', request()->query()) }}"
               target="_blank"
        ...
        <form method="GET" action="{{ route('admin.reports.expenses-by-category') }}" class="rounded-xl border border-bdr bg-surface p-4">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-bdr bg-bg/50">
                            ...
                            <th class="px-4 py-3 text-right font-semibold text-muted">الإجمالي</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-bdr">
                        @forelse ($expenses as $expense)
                            <tr class="transition-colors hover:bg-bg/50">
                                <td class="px-4 py-3 font-semibold text-text">{{ $expense->name }}</td>
                                <td class="px-4 py-3 font-semibold text-text">{{ number_format($expense->total, 2) }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="2" class="px-4 py-12 text-center text-muted">
                                    لا توجد بيانات صرف
                                </td>
                            </tr>
                        @endforelse
