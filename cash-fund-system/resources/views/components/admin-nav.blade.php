...
<nav class="mb-6 flex items-center gap-1 rounded-xl border border-bdr bg-surface p-1.5" aria-label="قائمة إدارة النظام">
    ...
    <a href="{{ route('admin.orders.index') }}"
       class="inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold transition-colors {{ str_starts_with($currentRoute, 'admin.orders') ? 'bg-primary text-white shadow-sm' : 'text-muted hover:bg-bg hover:text-text' }}">
        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
